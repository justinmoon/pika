...
- the current remaining blockers are no longer in the Incus executor dataplane:
  - `pre-merge-agent-contracts` is still backend-independent red: `agent-microvm-tests` fail on
    the same `pika-agent-microvm` assertions that also fail locally under
    `cargo test -p pika-agent-microvm --lib`
  - `pre-merge-pikachat-openclaw-e2e` still fails on Incus, but the blocker has moved up the
    staged runtime contract stack:
    the guest-local Node runtime now works, and the old `libsimdjson.so.29` crash is gone, but the
    staged OpenClaw `pnpm` tree still contains package-layout links that resolve back to the host
    store, which currently surfaces as missing module failures like `Cannot find module 'hasown'`
- because of that, the current honest migration read is:
  ...
  explicit exceptions kept off the automatic Incus path:
