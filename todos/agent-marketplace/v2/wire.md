...
This spec defines the CLI <-> server wire control plane for provisioning and managing coding-agent leases over Nostr.

Locked product decisions:
1. Use custom protocol events (not DVM/NIP-90) for now.
2. Offers are markdown-first; avoid over-structuring server internals in wire schema.
3. Client selects an offer; server chooses internal runtime/backend implementation.
4. v0 wire schema is intentionally minimal and backend-agnostic.
5. For this branch, wire protocol usage is ACP-only.

Non-goals for v0:
- embedding backend-specific runtime knobs in order payloads
- embedding raw Dockerfile/Nix build definitions in default order flow
...
Target payload families for v2:
- `agent.offer.v0`
...
Do not introduce backend taxonomy as required wire fields in v0.
...
Server chooses internal backend/workload implementation. Client does not select backend details in v0.
...
5. Optional capability/distribution layer as additive extension (see distribution draft).
