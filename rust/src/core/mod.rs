...
impl ProfileCache {
    ...
    fn display_picture_url(&self, data_dir: &str, pubkey_hex: &str) -> Option<String> {
        if self.picture_url.is_some() {
            ...
            if let url @ Some(_) = profile_pics::file_url_with_mtime(&path) {
                return url;
            }
        }
    ...
    /// Group pictures are encrypted, so we never return the raw remote URL.
    fn display_group_picture_url(
        ...
        if self.picture_url.is_some() {
            ...
            return profile_pics::file_url_with_mtime(&path);
        }
        None
    }
}
...
impl AppCore {
    ...
    fn upsert_group_profile(
        &mut self,
        chat_id: &str,
        pubkey: String,
        new: ProfileCache,
        encrypted_pic_info: Option<(String, String, String)>,
    ) {
        ...
        // Skip download if the file is already cached locally (e.g. we just uploaded it).
        if let Some(url) = need_pic_download {
            let already_cached =
                profile_pics::group_cached_path(&self.data_dir, chat_id, &pubkey).exists();
            if !already_cached {
    ...
    fn handle_peer_profile_fetched(
        ...
        // If a group profile exists, use it entirely; otherwise use global.
        if let Some(ref mut pp) = self.state.peer_profile {
            if pp.pubkey == pubkey {
                let chat_id = self.state.current_chat.as_ref().map(|c| c.chat_id.as_str());
                let group_cached = chat_id
                    .and_then(|cid| self.group_profiles.get(cid))
                    .and_then(|m| m.get(&pubkey));

                if let Some(gp) = group_cached {
                    pp.name = gp.name.clone();
                    pp.about = gp.about.clone();
                    pp.picture_url = chat_id
                        .and_then(|cid| gp.display_group_picture_url(&self.data_dir, cid, &pubkey));
                } else {
                    let global = self.profiles.get(&pubkey);
                    pp.name = global.and_then(|p| p.name.clone());
                    pp.about = global.and_then(|p| p.about.clone());
                    pp.picture_url =
                        global.and_then(|p| p.display_picture_url(&self.data_dir, &pubkey));
                }
                self.emit_state();
            }
        }
    ...
    fn handle_app_message(&mut self, chat_id: &str, msg: message_types::Message) {
        ...
        match kind {
            ...
            AppMessageKind::GroupProfile => {
                ...
                // Extract encrypted picture metadata from imeta tag transiently
                // (not stored in ProfileCache — only needed for the download).
    ...
    fn handle_action(&mut self, action: AppAction) {
        match action {
            ...
            AppAction::WipeProfileCache => {
                ...
                profile_pics::clear_cache(&self.data_dir);
                self.refresh_all_from_storage();
                self.toast("Profile cache wiped");
            }
            ...
            AppAction::OpenPeerProfile { pubkey } => {
                ...
                // Use group profile if one exists (all-or-nothing), otherwise global.
                let chat_id = self.state.current_chat.as_ref().map(|c| c.chat_id.as_str());
                let group_cached = chat_id
                    .and_then(|cid| self.group_profiles.get(cid))
                    .and_then(|m| m.get(&pubkey));

                let (name, about, picture_url) = if let Some(gp) = group_cached {
                    let pic = chat_id
                        .and_then(|cid| gp.display_group_picture_url(&self.data_dir, cid, &pubkey));
                    (gp.name.clone(), gp.about.clone(), pic)
                } else {
                    let global = self.profiles.get(&pubkey);
                    (
                        global.and_then(|p| p.name.clone()),
                        global.and_then(|p| p.about.clone()),
                        global.and_then(|p| p.display_picture_url(&self.data_dir, &pubkey)),
                    )
                };
...
mod tests {
    ...
    fn upsert_group_profile_inserts_new() {
        ...
        core.upsert_group_profile("chat1", "alice_pk".to_string(), cache, None);
    ...
    fn upsert_group_profile_skips_older() {
        ...
        core.upsert_group_profile("chat1", "pk1".to_string(), newer, None);
        ...
        core.upsert_group_profile("chat1", "pk1".to_string(), older, None);
    ...
    fn upsert_group_profile_does_not_affect_global() {
        ...
        core.upsert_group_profile("chat1", "alice".to_string(), group, None);
    ...
    fn upsert_group_profile_persists_to_db() {
        ...
        core.upsert_group_profile("chat1", "pk1".to_string(), cache, None);
    ...
    fn upsert_group_profile_separate_chats() {
        ...
        core.upsert_group_profile("chat1", "pk1".to_string(), cache1, None);
        core.upsert_group_profile("chat2", "pk1".to_string(), cache2, None);
    ...
    mod group_profile_tests {
        ...
        #[test]
        fn group_profiles_lazy_loaded_on_refresh() {
            let (mut core, chat_id, _keys, _gid) = make_core_with_group();
            core.refresh_all_from_storage();

            // Seed a group profile directly in the DB.
            let cache = ProfileCache::from_metadata_json(
                Some(r#"{"display_name":"DB Only"}"#.to_string()),
                1,
                0,
            );
            if let Some(conn) = core.profile_db.as_ref() {
                profile_db::save_group_profile(conn, "lazy_pk", &chat_id, &cache);
            }

            // Ensure it's NOT in memory yet.
            assert!(
                core.group_profiles
                    .get(&chat_id)
                    .and_then(|m| m.get("lazy_pk"))
                    .is_none(),
                "profile should not be in memory before refresh"
            );

            // Clear the in-memory map so the lazy-load path is triggered.
            core.group_profiles.clear();

            // refresh_chat_list_from_storage should lazy-load from DB.
            ...
            let gp = core
                ...
                .get(&chat_id)
                .and_then(|m| m.get("lazy_pk"));
            assert!(gp.is_some(), "group profile should be lazy-loaded from DB");
            assert_eq!(gp.unwrap().name.as_deref(), Some("DB Only"));
        }
        ...
        fn stop_session_clears_group_profiles() {
            ...
            core.upsert_group_profile(&chat_id, "pk1".to_string(), cache, None);
