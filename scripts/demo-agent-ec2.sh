...
SPAWN_VARIANT="${SPAWN_VARIANT:-legacy}"
...
BOOTSTRAP_KIND="${BOOTSTRAP_KIND:-nixos-config}"
...
if [[ "$SPAWN_VARIANT" != "legacy" && "$SPAWN_VARIANT" != "prebuilt" && "$SPAWN_VARIANT" != "prebuilt-cow" ]]; then
  echo "SPAWN_VARIANT must be legacy, prebuilt, or prebuilt-cow."
  exit 1
fi
...
if ! spawner_reachable; then
  if [[ "$AUTO_TUNNEL" == "1" && "$SPAWNER_URL" == "$DEFAULT_SPAWNER_URL" ]]; then
    ...
    TUNNEL_LOG="$(mktemp -t pika-ec2-tunnel.XXXXXX.log)"
...
cmd+=(
  ...
  --bootstrap-kind "$BOOTSTRAP_KIND"
)

if [[ -n "$BOOTSTRAP_REF" ]]; then
  cmd+=(--bootstrap-ref "$BOOTSTRAP_REF")
fi
if [[ -n "$IMAGE_REF" ]]; then
...
echo "Running EC2-style agent demo (backed by vm-spawner scaffold)..."
status=0
if "${cmd[@]}"; then
  status=0
else
