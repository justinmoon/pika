...
pub enum ScenarioCommand {
    ...
    /// Phase 4: Rust harness invites a peer pubkey (e.g. OpenClaw pikachat channel) and asserts a strict reply
    InviteAndChatPeer {
...
pub async fn cmd_scenario(cli: &Cli, scenario: &ScenarioCommand) -> anyhow::Result<()> {
    let relay = harness_relay(cli);
    match scenario {
        ScenarioCommand::InviteAndChat {
            timeout_sec,
            giftwrap_lookback_sec,
        } => scenario_invite_and_chat(&relay, &cli.state_dir, *timeout_sec, *giftwrap_lookback_sec)
            .await
            .context("scenario invite-and-chat failed"),
        ScenarioCommand::InviteAndChatRustBot { timeout_sec } => {
            scenario_invite_and_chat_rustbot(&relay, &cli.state_dir, *timeout_sec)
                .await
                .context("scenario invite-and-chat-rust-bot failed")
        }
        ...
        ScenarioCommand::AudioEcho { frames } => {
            let stats = pikachat_sidecar::daemon::run_audio_echo_smoke(*frames)
                .await
...
fn harness_relay(cli: &Cli) -> String {
    cli.relay
        .first()
        .cloned()
...
async fn scenario_invite_and_chat_daemon(
    ...
    ensure_dir(state_dir).context("create state dir")?;
    let a_state = state_dir.join("a");
    let d_state = state_dir.join("pikachat");
    ensure_dir(&a_state)?;
    ensure_dir(&d_state)?;
