...
16. Slice 17 promoted one late-joiner group-profile visibility path into a real selector-owned contract:
   - `crates/pikahut/tests/integration_deterministic.rs::late_joiner_group_profile_visibility_after_explicit_refresh_boundary` now gives the post-join explicit-refresh path a readable deterministic CI-facing owner instead of leaving it only in `rust/tests/e2e_group_profiles.rs`
...
24. Slice 25 finished the remaining obvious signer/auth UX ownership seams:
   ...
   - `rust/tests/app_flows.rs` was narrowed accordingly, but not emptied of exercised coverage: the direct external-signer and direct bunker success/failure paths now remain as small default-exercised Rust smokes for hint/error/descriptor plumbing, while `pikahut` owns the fuller readable lifecycle contracts
   - this effectively finishes the signer/auth family cleanup in ownership terms: `pikahut` now owns the readable user-facing signer lifecycle contracts, while Rust keeps the smaller default-exercised smokes plus the lower-level callback-gating, retry-branch, persisted-pairing, stored-key, descriptor, and current-user plumbing semantics
...
25. Slice 26 made the late-joiner rebroadcast caveat explicit instead of implying a stronger contract than the code proves:
   - both checked-in owners are now named honestly around the weaker contract we actually have today: `crates/pikahut/tests/integration_deterministic.rs::late_joiner_group_profile_visibility_after_explicit_refresh_boundary` and `rust/tests/e2e_group_profiles.rs::post_join_group_profile_refresh_reaches_late_joiner_member_state`
...
7. Obvious redundancies and gaps in this area today:
   ...
   - the main user-facing message/profile flows now have selector-owned deterministic `pikahut` contracts; the remaining caveat is explicit rather than confusing: true pre-existing late-joiner rebroadcast is still deferred because current checked-in code rebroadcasts before the late joiner subscribes, so the suite only owns the weaker explicit-refresh contract today
   - the single-app auth/session family is coherent, and the signer family is now effectively closed in ownership terms: the readable signer UX contracts live in checked-in deterministic `pikahut` selectors, while lower-level external-signer current-user-hint/current-user and bunker descriptor/client-key plumbing, plus small default-exercised signer smokes, remain Rust-only by design
